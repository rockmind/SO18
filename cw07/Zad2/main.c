...
void koniec(){
	...
	sem_destroy(&semBlockers);
    sem_destroy(&semProducer);
    sem_destroy(&semKonsumer);
...
/*8*/	nk        = atoi(configLine); 
	...
	sem_init(&semBlockers,0,1);
	sem_init(&semProducer,0,0);
	sem_init(&semKonsumer,0,1);
