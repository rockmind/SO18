/* program wypisujcy czasy wykonania programow podanych jako kolejne argumenty, powinien wypisac czas rzeczywisty, uzytkownika i systemwy*/
...
do_cmd(char *cmd,int instruction, int final)		/* wykonaj program, policz jego czasy, sprawdz, czy to nie jest ostatni program */
{
	...
	if (/*DO ZROBIENIA: zapamietaj czas w chwili aktualnej w zmiennych start i tmsstart*/(start=times(&tmsstart)) == -1)	        /* zapamietaj wartosci poczatkowe pomiaru czasu*/
		{printf("times error"); exit(-1);}

	if ((status = system(cmd)) < 0)			/* wykonaj polecenie */
		{printf("system() error");exit(-1);}

	if (/*DO ZROBIENIA: zapamietaj czas w chwili aktualnej w zmiennych end i tmsend*/(end=times(&tmsend)) == -1)		/* zapamietaj wartosci koncowe */
		{printf("times error"); exit(-1);}
